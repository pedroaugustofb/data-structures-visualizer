...
interface LinkedListCanvastProps extends React.HTMLAttributes<HTMLDivElement> {
...
export default function LinkedListCanvas({
  ...
  const secondArrowPosition = calculateArrowPosition(FINAL_ITEM_POSITION, PREV_ITEM_POSITION);
  const secondArrowDirection = calculateArrowDirection(FINAL_ITEM_POSITION, PREV_ITEM_POSITION);
  ...
  return (
    <mesh ref={meshRef} position={INITIAL_POSITION}>
      ...
      {index > 0 && (
        <arrowHelper
          args={[arrowDirection, arrowPosition, arrowPosition.length() - 0.55, hex, 0.1, 0.1]}
          position={structure === "double_linked_list" ? [-1.85, -0.1, -0.5] : undefined}
        />
      )}
      {index > 0 && structure == "double_linked_list" && (
        <arrowHelper
          args={[secondArrowDirection, secondArrowPosition, secondArrowPosition.length() - 0.55, hex, 0.1, 0.1]}
          position={[0, 0.1, -0.5]}
        />
      )}
